...
void ScreenRecoveryUI::SetColor(UIElement e) const {
  switch (e) {
    ...
    case UIElement::MENU:
      gr_color(0x39, 0xc5, 0xbb, 255);
      break;
    case UIElement::MENU_SEL_BG:
    case UIElement::SCROLLBAR:
      if (fastbootd_logo_enabled_)
        gr_color(0xf3, 0x8f, 0xbc, 255);
      else
        gr_color(0x32, 0xcf, 0xea, 255);
      break;
...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    auto& logo = fastbootd_logo_enabled_ ? fastbootd_logo_ : miku_logo_;
...
bool ScreenRecoveryUI::Init(const std::string& locale) {
  ...
  if (android::base::GetBoolProperty("ro.boot.dynamic_partitions", false) ||
    ...
    miku_logo_ = LoadBitmap("logo_image_switch");
    fastbootd_logo_ = LoadBitmap("fastbootd");
  } else {
    miku_logo_ = LoadBitmap("logo_image");
  }
...
int ScreenRecoveryUI::SelectMenu(const Point& p) {
  ...
  if (menu_) {
    if (!menu_->IsMain()) {
      // Back arrow hitbox
      const static int logo_width = gr_get_width(miku_logo_.get());
      const static int logo_height = gr_get_height(miku_logo_.get());
